...
| **A8** | 🟡 **"Fin de turno" (R3)** — popup en el marker (mismo patrón que `crearContenidoPopup` en `Semaforo.tsx`), visible solo si la unidad está `libre` en A1 (oculto/deshabilitado si `en_proceso`). Reusa el endpoint `DELETE /api/ambulance/[id]/simulate` y el canal `ambulancias-detenidas` **tal cual existen hoy** — con el nuevo modelo, ese canal deja de tener el significado ambiguo de "este viaje terminó" (que hoy es el origen del bug) y pasa a significar exactamente "esta unidad se retiró", que es información durable y sin ambigüedad. | |
