...
| **A6** | 🟡 **Endpoint de ingestión GPS real** — `POST /api/ambulance/[id]/position` (`{lat, lng, timestamp}`): valida, publica al canal Portal **propio de ese `ambulanceId`** (A1, no compartido), y si es la primera posición de ese id, dispara el cálculo de destino (A3) usando esa posición como origen — mismo flujo que un click simulado. | |
...
Slices #13, #14 y #15 cerrados e implementados (ver commits referenciados en los issues). Queda #16 (GPS pluggable) — su mecanismo (A5/A6) sigue vigente, solo A6 necesita publicar al canal propio del `ambulanceId` (R8) en vez del canal compartido que asumía originalmente.
